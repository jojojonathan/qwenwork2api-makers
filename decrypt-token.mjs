...
// 用法: node decrypt-token.mjs <auth-v2.dat> <aes-key-hex> [out.json|-]（- 或缺省=只校验不写文件）
...
if (outPath && outPath !== '-') {
  ...
  console.log('✓ auth-v2.dat 校验通过（可解密，字段完整）');
}
console.log(`  用户: ${j.user.name || j.user.username || j.user.id}`);
console.log(`  token 过期: ${j.expiresAt || '未知'}（约 7 天有效期，客户端会自动刷新，容器实时跟进）`);
