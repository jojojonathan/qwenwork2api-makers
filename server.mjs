...
// 架构：解密客户端 token(挂载) + 官方 WASM 签名 + business.sub_task 路由
...
const TOKEN_FILE = process.env.QW2A_TOKEN_FILE || '/data/token.json';
const MACHINE_ID_FILE = process.env.QW2A_MACHINE_ID_FILE || '/data/machine-id';
const WASM_PATH = process.env.QW2A_WASM || '/app/qoder_auth_wasm_bg.wasm';
const ORIGIN = 'https://gateway.qwenwork.cn';
const COSY_VERSION = '1.1.59';
...
function loadWasm() {
  ...
  const inst = new WebAssembly.Instance(new WebAssembly.Module(buf), buildImports());
  Pn = inst.exports;
  ex = Pn;
}
...
// ---------- token / 上下文管理 ----------
let curToken = null, curCtx = 0, tokenMtime = 0;
...
function readToken() {
  ...
  if (!auth.token || !auth.user || !auth.user.id) throw new Error('token.json 格式无效（缺少 token/user.id）');
  curToken = auth;
...
function readMachineId() {
  try { return fs.readFileSync(MACHINE_ID_FILE, 'utf8').trim(); } catch { return crypto.randomUUID(); }
}

function ensureCtx() {
  const auth = readToken();
  if (curCtx && curToken === auth) return curCtx;
  return withWasm(() => {
    if (curCtx && curToken === auth) return curCtx;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (url.pathname === '/' && req.method === 'GET') {
      return sendJson(res, 200, { service: 'qwenwork2api', status: 'ok', models: Object.keys(MODELS) });
    }
    ...
    if (url.pathname === '/v1/chat/completions' && req.method === 'POST') {
      ...
      try { auth = readToken(); } catch (e) {
        return sendJson(res, 503, { error: { message: `token 不可用：${e.message}。请重新运行 docker-run.sh 从客户端同步最新 token`, type: 'server_error' } });
      }
      // JWT 过期预检
      try {
        const jwt = JSON.parse(Buffer.from(auth.token.split('.')[1], 'base64url').toString('utf8'));
        if (jwt.exp && jwt.exp * 1000 < Date.now()) {
          return sendJson(res, 401, { error: { message: 'token 已过期，请重新运行 docker-run.sh 同步最新 token', type: 'invalid_api_key' } });
        }
      } catch {}
...
try {
  ...
  console.error('[qwenwork2api] 请先运行 docker-run.sh（会从 Windows 客户端同步 qoder_auth_wasm_bg.wasm）');
  process.exit(1);
}
try { readToken(); } catch (e) {
  console.error(`[qwenwork2api] token 加载失败（${TOKEN_FILE}）: ${e.message}`);
  console.error('[qwenwork2api] 请先运行 docker-run.sh 解密 token');
}
