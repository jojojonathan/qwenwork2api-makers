...
// 4 参数构造：machineId, cosyVersion, userInfoJson, clientMetaJson
const nc4=(mid,cv,ui,meta)=>{const sp=stackAdj(-16);try{const ps=[mid,cv,ui,meta].map(a=>passStr(a,malloc,realloc));ex.qodercontext_new(sp,ps[0].ptr,ps[0].len,ps[1].ptr,ps[1].len,ps[2].ptr,ps[2].len,ps[3].ptr,ps[3].len);const dv=DV();const p=dv.getInt32(sp,true),e=dv.getInt32(sp+4,true),f=dv.getInt32(sp+8,true);if(f){const x=getHeap(e);throw new Error(x&&x.message?x.message:String(x));}return p>>>0;}finally{stackAdj(16);}};
const prep=(c,u,b,m,s)=>{const sp=stackAdj(-16);try{const ps=[u,b,m,s].map(x=>passStr(x,malloc,realloc));ex.qodercontext_prepareInferRequest(sp,c,ps[0].ptr,ps[0].len,ps[1].ptr,ps[1].len,ps[2].ptr,ps[2].len,ps[3].ptr,ps[3].len);const dv=DV();const p=dv.getInt32(sp,true),e=dv.getInt32(sp+4,true),f=dv.getInt32(sp+8,true);if(f){const x=getHeap(e);throw new Error(x&&x.message?x.message:String(x));}return p>>>0;}finally{stackAdj(16);}};
...
const metas = {
  'CN(qoder_work/6)': JSON.stringify({client_type:'6',business_product:'qoder_work',business_type:'agent',scene:'assistant'}),
  'cli/5(旧默认)': JSON.stringify({client_type:'5',business_product:'cli',business_type:'agent',scene:'assistant'}),
  'qwenwork/6': JSON.stringify({client_type:'6',business_product:'qwenwork',business_type:'agent',scene:'assistant'}),
};
for (const [name, meta] of Object.entries(metas)) {
  try {
    const ctx = nc4(MID, '1.1.59', userInfo, meta);
    ...
    const r = await fetch(url, {method:'POST',headers:hdrs,body:ob,signal:AbortSignal.timeout(35000)});
    const t = await r.text();
    const m = t.match(/message\\?":\\?"([^\\"]+)/);
    console.log(`[${name}] BP=${hdrs['Cosy-Business-Product']} CT=${hdrs['Cosy-ClientType']} → ${r.status} ${m?m[1]:t.slice(0,80).replace(/\n/g,' ')}`);
  } catch(e) { console.log(`[${name}] ERR ${e.message.slice(0,90)}`); }
}
