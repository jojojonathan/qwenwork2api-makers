...
// 严格按 SDK 的构造：chat_context.modelConfig 只有 {key,is_reasoning}；含 custom_model
function mkBody() {
  ...
  return JSON.stringify({
    ...
    chat_context: { text, features: [], extra: { context: [], modelConfig: { key: 'qwen3.8-max-preview', is_reasoning: true }, originalContent: text }, chatPrompt: '', imageUrls: null },
    ...
    model_config: { key: 'qwen3.8-max-preview', display_name: 'Qwen3.8-Max', model: '', format: 'openai', is_vl: true, is_reasoning: true, api_key: '', url: '', source: 'system', max_input_tokens: 1000000 },
    custom_model: null,
    system: '', messages: [{ role: 'user', content: text }], tools: [], parameters: { max_tokens: 16 },
  });
}
const body = mkBody();
...
const r = await fetch(url, {method:'POST',headers:hdrs,body:ob,signal:AbortSignal.timeout(40000)});
